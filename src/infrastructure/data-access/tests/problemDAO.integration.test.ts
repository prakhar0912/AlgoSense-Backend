...
function buildProblemInput(label: string, difficulty: Problem["difficulty"] = "medium"): ProblemInsertPayload {
  return {
    ...
    similar_problems: [`${runId}-${label}-similar-1`, `${runId}-${label}-similar-2`],
    difficulty,
...
describe("ProblemDAO integration", () => {
  it("round-trips create, find, list, update, and delete against PostgreSQL", async () => {
    ...
    expect(alpha.similar_problems).toEqual(alphaInput.similar_problems)
    ...
    expect(lookedUpAlpha).toMatchObject({
      ...
      similar_problems: alphaInput.similar_problems,
    ...
    expect(lookedUpByName?.similar_problems).toEqual(muInput.similar_problems)

    const filteredBySimilarProblems = await measure(
      metrics.listMs,
      () => problemDAO.list({ similar_problems: alphaInput.similar_problems }, 1, 5),
    )
    expect(filteredBySimilarProblems.pagination).toEqual({ page: 1, perPage: 5 })
    expect(filteredBySimilarProblems.data).toHaveLength(1)
    expect(filteredBySimilarProblems.data[0]?.id).toBe(alpha.id)
    ...
    const updatedSimilarProblems = [`${runId}-mu-updated-similar-1`, `${runId}-mu-updated-similar-2`]
    const updatedApproaches = [buildApproach(2, {
    ...
    const updatedInput: Partial<Problem> = {
      ...
      similar_problems: updatedSimilarProblems,
      difficulty: "hard",
    ...
    expect(updatedMu.similar_problems).toEqual(updatedSimilarProblems)
    ...
    expect(lookedUpUpdatedName?.similar_problems).toEqual(updatedSimilarProblems)
  ...
  it("returns null, empty pages, false deletes, and write failures for missing problems", async () => {
    ...
    await expect(measure(metrics.updateMs, () => problemDAO.update(missingProblemId, {
      title: `${runId}-ghost`,
      similar_problems: [`${runId}-ghost-similar`],
