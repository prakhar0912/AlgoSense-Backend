...
const systemPromptCreator = (problem) => {
    let expectedApproach;
    let otherApproaches = [];
    let approachesStrings = [];
    let edgeCases = [];
    ...
    const { primary_technique, time_complexity, space_complexity, req_or_constraints, steps, explanation, edge_cases, } = expectedApproach;
...
export default async (problem, userInput) => {
    ...
    const modelResp = JSON.parse(data.choices[0].message.content);
    return modelResp;
};
let problem = {
    id: "1",
...
//# sourceMappingURL=askGPT.js.map