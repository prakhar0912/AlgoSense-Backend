...
export default class GetProblemBySlugForUser implements IUseCase<Omit<Problem, 'approaches' | 'hints' | 'evaluation_criteria'>> {
  constructor(
    private problemDAO: IProblemDAO
  ) { }
  async call(problemSlug: string): Promise<Omit<Problem, 'approaches' | 'hints' | 'evaluation_criteria'>> {
    ...
    try {
      problem = await this.problemDAO.findBySlugForUsers(problemSlug)
    }
    catch (e) {
